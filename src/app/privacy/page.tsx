...
export default function PrivacyPolicySection() {
  const lastUpdated = "August 26, 2025"; // 🔁 Update when you change the policy
  ...
  const sections = [
    ...
    { title: "Information We Collect", content: (
      <ul className="list-disc list-inside space-y-1 text-gray-200">
        <li><strong>Personal Information:</strong> name, email, phone number, postal address, passport details (for Hajj/Umrah processing), and payment details (processed by secure third parties).</li>
        <li><strong>Usage Data:</strong> IP address, device and browser type, pages viewed, time spent, and referring pages.</li>
        <li><strong>Cookies &amp; Similar Technologies:</strong> used for authentication, preferences, analytics, and marketing. You can control cookies through your browser settings.</li>
      </ul>
    )},
    ...
    { title: "Legal Bases for Processing (GDPR)", content: (
      <ul className="list-disc list-inside space-y-1 text-gray-200">
    ...
    { title: "Sharing of Information", content: (
      <>
    ...
    { title: "International Data Transfers", content: (
      <p>Your information may be transferred to and processed in countries other than your own. Where required, we use appropriate safeguards (such as standard contractual clauses) to protect your data.</p>
    )},
    { title: "Data Retention", content: (
      <p>We retain personal data only for as long as necessary to provide Services, comply with legal obligations, resolve disputes, and enforce agreements. Retention periods may vary by data type and legal requirements.</p>
    )},
    ...
    { title: "Cookies & Analytics", content: (
      <p>
    ...
    { title: "Advertising & Remarketing (Optional)", content: (
      <p>We may work with advertising partners to show relevant ads. Partners may set cookies or use similar technologies to measure ad performance. You can opt out via your browser or device settings.</p>
    )},
    { title: "Children’s Privacy", content: (
      <p>Our Services are not directed to children under 13 (or the age defined by local law). We do not knowingly collect data from children. If you believe a child has provided us data, contact us to delete it.</p>
    )},
    { title: "Data Security", content: (
      <p>We implement administrative, technical, and physical safeguards designed to protect personal information. However, no method of transmission over the Internet is 100% secure.</p>
    )},
    { title: "Third-Party Links", content: (
      <p>Our site may contain links to other websites. We are not responsible for the privacy practices of those websites. We encourage you to review their policies.</p>
    )},
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 container mx-auto px-6 max-w-5xl">
        ...
        >
          <h1 className={`${merienda.className} text-4xl md:text-5xl font-bold mb-4 bg-gradient-to-r from-amber-400 to-yellow-500 bg-clip-text text-transparent`}>
            Privacy Policy
          </h1>
          <p className="text-gray-300 italic">Last updated: {lastUpdated}</p>
        </motion.div>
        ...
        >
          {sections.map((s, idx) => (
            <PolicyCard key={idx} title={`${idx + 1}. ${s.title}`}>
              {s.content}
            </PolicyCard>
          ))}
        </motion.div>
...
function PolicyCard({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <motion.article
      variants={fadeUp}
      className="relative overflow-hidden rounded-2xl border border-white/10 bg-white/10 backdrop-blur-xl p-6 md:p-8 shadow-2xl 
      hover:border-amber-400/50 hover:shadow-amber-400/30 hover:scale-[1.01] transition-all"
    >
      <h2 className={`${merienda.className} text-2xl font-bold mb-4 text-amber-400`}>
        {title}
      </h2>
